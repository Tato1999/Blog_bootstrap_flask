...
db = SQLAlchemy(app)
gravatar = Gravatar(app, size=60, rating='g', default='retro', force_default=False, force_lower=False, use_ssl=False, base_url=None)
...
class BlogUser(UserMixin, db.Model, Base):
    ...
    comments = relationship('BlogComment', backref='com_author')
...
class BlogPost(db.Model, Base):
    ...
    parent_of_comment = relationship("BlogComment", backref='com_aut')
...
class BlogComment(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    author_id = db.Column(db.String, ForeignKey('blog_user.id'))
    post_id = db.Column(db.String, ForeignKey('blog_post.id'))
...
def show_post(index):
    ...
    comments = db.session.query(BlogComment).all()
        ...
        if form.validate_on_submit():
            if request.method == "POST":
                try:
                    data = request.form
                    print(data['body'])
                    comment_object = BlogComment(text = data['body'],author_id = current_user.id, post_id = index)
                    db.session.add(comment_object)
                    db.session.commit()
                    comments = db.session.query(BlogComment).all()
                    return render_template("post.html", post=requested_post, form = form, comment = comments)
                except AttributeError:
                    return render_template('login.html', txt = 'Login Required')
    return render_template("post.html", post=requested_post, form = form, comment = comments)
...
# # CONNECT TO DB
# app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://postgres:G1234567@@localhost/blog_post_db'
# app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False
# db = SQLAlchemy(app)
...
#     author = relationship('BlogUser', backref='posts')
...
#         mail = data['mail']
#         password = data['password']
#         get_info = BlogUser.query.filter_by(mail=mail).first()
#         if get_info:
#             if get_info.password and check_password_hash(pwhash=get_info.password, password=password):
#                 print('yep')
#                 login_user(get_info)
...
#         password = request.form['password']
#         gen_hash = generate_password_hash(password, salt_length=8, method="pbkdf2:sha256")
#         user_info = BlogUser(permission=1, name=name, second_name=lastName, mail=mail, password=gen_hash)
#         user_mail = mail
#         base_mail = BlogUser.query.filter_by(mail=mail).first()
#         print(base_mail)
#         if base_mail:
#             print('error')
#             txt = 'This Mail is Already Used'
...
#             requested_post.subtitle = edit_form.data['subtitle']
#             requested_post.img_url = edit_form.data['img_url']
#             requested_post.author.name = edit_form.data['author']
#             requested_post.body = edit_form.data['body']
#             db.session.commit()
#             return redirect(url_for('get_all_posts'))
#     return render_template("edit.html", form=edit_form)
...
#     return render_template("check.html", txt="Maybe Something Went Wrong. Try Sending the Mail Again")
